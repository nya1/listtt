...
const STATE_ORDER = { waiting: 0, busy: 1, idle: 2, other: 2, ended: 3, archived: 4 };
const STATE_LABEL = { waiting: 'Needs input', busy: 'Working', idle: 'Idle', ended: 'Ended', archived: 'Archived' };
...
export function visibleSessions(snap, ui) {
  ...
  return snap.sessions.filter((s) => {
    if (ui.view === 'archived') {
      ...
      if (ui.view.startsWith('g_') && s.groupId !== ui.view) return false;
    }
...
export function renderSidebar(snap, ui) {
  const current = snap.sessions.filter((s) => s.state !== 'archived');
  ...
  let html = item('all', 'All', current);
  html += item('ungrouped', 'Ungrouped', current.filter((s) => s.groupId === ''), { dropGroup: '' });
  for (const g of snap.groups) {
    html += item(g.id, g.name, current.filter((s) => s.groupId === g.id), { dropGroup: g.id, groupId: g.id });
  }
  html += '<li class="nav-sep"></li>';
  html += item('archived', 'Archived', snap.sessions.filter((s) => s.state === 'archived'));
  return html;
}
...
export function renderRow(s, snap, ui, now, dropGroup) {
  ...
  if (s.state === 'ended' || s.state === 'archived') {
...
export function renderRows(snap, ui, now) {
  ...
  const empty = '<tr class="empty-row"><td colspan="7">No sessions match.</td></tr>';
  if (ui.view !== 'all') {
  ...
  for (const g of [{ id: '', name: 'Ungrouped' }, ...snap.groups]) {
    ...
    html += `<tr class="group-row" data-action="toggle-group" data-group="${esc(g.id)}" data-drop-group="${esc(g.id)}">` +
      `<td colspan="7"><span class="caret">${collapsed ? '▸' : '▾'}</span> <strong>${esc(g.name)}</strong> ` +
